...
const directionText = Object.freeze({ A_MORE_BOUNDARY_SENSITIVE:'서로의 경계 기준을 함께 확인하면 편안한 편', B_MORE_BOUNDARY_SENSITIVE:'서로의 경계 기준을 함께 확인하면 편안한 편', SIMILAR_BOUNDARY:'관계의 경계 기준이 비슷한 편', A_MORE_INTIMACY_PACE:'마음을 여는 속도를 서로 존중할 필요가 있는 편', B_MORE_INTIMACY_PACE:'마음을 여는 속도를 서로 존중할 필요가 있는 편', SIMILAR_INTIMACY_PACE:'마음을 여는 속도가 비슷한 편' });
export function buildCoupleResultViewModel(input) {
  ...
  if (d.D6) map.push({ id:'D6_BOUNDARY', name:'관계의 경계 기준', mode:directionText[d.D6.boundary_direction] ?? '서로의 기준을 살피는 편' }, { id:'D6_INTIMACY', name:'마음을 여는 속도', mode:directionText[d.D6.intimacy_direction] ?? '서로의 속도를 살피는 편' });
